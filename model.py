...
class CasualSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        #kqv for all heads in a single batch
    ...
    def forward_pass(self, x):
        ...
        k = k.view(batch_size, seq_len, self.n_head, embed_dim // self.n_head).transpose(1, 2) #(batch_size, nh, seq_length, hs)
        ...
        y = attn @ v #(batch_size, nh, seq_len, hs)
...
class Model(nn.Module):
    ...
    def forward_pass(self, idx):
        ...
        pos = torch.arange(0, seq_len, dtype = torch.long, device = idx.device) #(seq_len)
        pos_embed = self.transformer.wpe(pos) # (seq_len, n_embd)
